...
import UpdateQuizAnswerInfoInput from './UpdateQuizAnswerInfo.js';

const UpdateQuizQuestionInfoInput = new GraphQLInputObjectType({
  name: 'UpdateQuizQuestionInfoInput',
  description: 'Input for updating a quiz question. A null id means the question is new.',
  fields: {
    id: {
      type: GraphQLID,
      description: 'The ID of the question to update, null for a new question.',
    },
    prompt: {
    ...
    answers: {
      type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(UpdateQuizAnswerInfoInput))),
      description: 'The full list of answers. Answers omitted from the list are deleted.',
    },
  },
});

export default UpdateQuizQuestionInfoInput;
