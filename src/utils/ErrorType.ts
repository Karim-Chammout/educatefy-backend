export enum ErrorType {
  INTERNAL_SERVER_ERROR = 'INTERNAL_SERVER_ERROR',
  SIGN_UP_FIRST = 'SIGN_UP_FIRST',
  INVALID_URL = 'INVALID_URL',
  INVALID_STATE = 'INVALID_STATE',
  NOT_FOUND = 'NOT_FOUND',
  NOT_AUTHORIZED = 'NOT_AUTHORIZED',
  UNAUTHENTICATED = 'UNAUTHENTICATED',
  FORBIDDEN = 'FORBIDDEN',
  PERMISSION_DENIED = 'PERMISSION_DENIED',
  OIDC_NOT_FOUND = 'OIDC_NOT_FOUND',
  OIDC_INITIALIZE_FAILED = 'OIDC_INITIALIZE_FAILED',
  OIDC_INVALID_PROVIDER = 'OIDC_INVALID_PROVIDER',
  NO_FILE_UPLOADED = 'NO_FILE_UPLOADED',
  NO_FILE_PATH_PROVIDED = 'NO_FILE_PATH_PROVIDED',
  UPLOADING_FILE_FAILED = 'UPLOADING_FILE_FAILED',
  INVALID_FORM_VALUES = 'INVALID_FORM_VALUES',
  INVALID_INPUT = 'INVALID_INPUT',
  INVALID_SUBJECTS = 'INVALID_SUBJECTS',
  INVALID_COURSE_LINKING = 'INVALID_COURSE_LINKING',
  UPDATING_PROFILE_PIC_FAILED = 'UPDATING_PROFILE_PIC_FAILED',
  NOT_SUPPORTED_LANGUAGE = 'NOT_SUPPORTED_LANGUAGE',
  INVALID_SLUG = 'INVALID_SLUG',
  SLUG_ALREADY_TAKEN = 'SLUG_ALREADY_TAKEN',
  MISSING_INPUT = 'MISSING_INPUT',
  ALREADY_ENROLLED = 'ALREADY_ENROLLED',
  NO_ENROLLMENT_FOUND = 'NO_ENROLLMENT_FOUND',
  PREREQUISITE_CYCLE_DETECTED = 'PREREQUISITE_CYCLE_DETECTED',
  INVALID_PREREQUISITE = 'INVALID_PREREQUISITE',
  PROGRAM_VERSION_EMPTY = 'PROGRAM_VERSION_EMPTY',
  NO_PUBLISHED_PROGRAM_VERSION_FOUND = 'NO_PUBLISHED_PROGRAM_VERSION_FOUND',
  NO_DRAFT_PROGRAM_VERSION_FOUND = 'NO_DRAFT_PROGRAM_VERSION_FOUND',
  DRAFT_ALREADY_EXISTS = 'DRAFT_ALREADY_EXISTS',
  ALREADY_ON_LATEST_VERSION = 'ALREADY_ON_LATEST_VERSION',
  TOO_MANY_REQUESTS = 'TOO_MANY_REQUESTS',
  MAX_ATTEMPTS_REACHED = 'MAX_ATTEMPTS_REACHED',
  MAX_SUBJECTS_EXCEEDED = 'MAX_SUBJECTS_EXCEEDED',
  MIN_SUBJECTS_REQUIRED = 'MIN_SUBJECTS_REQUIRED',
}
